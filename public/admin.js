/* Admin-Bereich: Turnier anlegen, Ergebnisse korrigieren, Phasen abschließen, Druck. */
(function () {
  'use strict';
  ...
  let gameFilter = { phase: 'all', field: 'all', open: false };
  ...
  async function refresh() {
    ...
    view = await res.json();
    if (view.empty) { formats = view.formats; view = null; }
    render();
  }
  ...
  function renderLogin(error) {
    app.innerHTML = `<div class="card" style="max-width:420px;margin:2rem auto"><h1>Anmeldung</h1>
      ...
      <p class="muted small">Die PIN wird beim Start des Servers über die Umgebungsvariable ADMIN_PIN gesetzt.</p></div>`;
    document.getElementById('login').addEventListener('submit', async (e) => {
      e.preventDefault();
      pin = new FormData(e.target).get('pin');
      try { await api('POST', '/api/admin/login', {}); sessionStorage.setItem('vt.pin', pin); await refresh(); } catch (err) { pin = ''; renderLogin(err.message); }
    });
  }
  ...
  function renderSetup() {
    ...
    document.getElementById('pasteBtn').addEventListener('click', () => {
      ...
      for (const line of lines) {
        const [g, name, club, contact] = line.split(/[;\t]/).map((s) => (s || '').trim());
        const gid = (g || '').toUpperCase();
        (byGroup[gid] = byGroup[gid] || []).push({ name, club, contact });
      }
      rows.forEach((r, i) => {
        const list = byGroup[r.group] || [];
        const t = list[r.idx - 1];
        if (!t) return;
    ...
    document.getElementById('setup').addEventListener('submit', async (e) => {
      ...
      try { view = await api('POST', '/api/admin/setup', body); section = 'status'; toast('Turnier angelegt.'); render(); } catch (err) { toast(err.message, true); }
    });
    document.getElementById('importFile').addEventListener('change', async (e) => {
      const file = e.target.files[0]; if (!file) return;
      try { const state = JSON.parse(await file.text()); view = await api('POST', '/api/admin/import', { state }); toast('Sicherung eingespielt.'); render(); } catch (err) { toast(err.message, true); }
    });
  }
  ...
  function renderStatus() {
    ...
    if (view.phase === 1) {
      ...
      const needsConfirm = view.rounds.some((r) => r.type === 'roundrobin');
      phaseHtml = `<p>Die 2. Phase läuft: <strong>${p[1].done} von ${p[1].total}</strong> Ergebnissen liegen vor.</p>
        ${view.phaseComplete[2] ? (needsConfirm ? '<p class="notice ok">Alle Spiele der 2. Phase sind gespielt. Prüfe die Platzierungen der Runden und schließe die Phase ab – dann stehen die Finalteilnehmer fest.</p>' + renderPlacements(2) : '<p class="notice ok">Alle Spiele der 2. Phase sind gespielt. Die Finalteilnehmer stehen automatisch fest. Bitte Phase abschließen.</p>' + renderPlacements(2)) : ''}
    ...
    return `<div class="card"><h2>${esc(t.name)}</h2>
      ...
      <p class="inline"><a class="btn" href="/print/qr" target="_blank">QR-Codes der Felder</a>
      ${t.fields.map((f) => `<a class="btn" href="/print/feld/${f.number}" target="_blank">Spielzettel ${esc(f.name)}</a>`).join(' ')}
      <a class="btn" href="/print/plan" target="_blank">Spielplan</a> <a class="btn" href="/print/tabellen" target="_blank">Tabellen</a></p>
      <p class="muted small">Die QR-Codes verweisen auf die Adresse, unter der diese Seite aufgerufen wurde (oder PUBLIC_URL). Die Schiri-Teams brauchen also nur WLAN/Mobilfunk-Zugang zu dieser Adresse.</p></div>`;
  }
  ...
  function renderPlacements(phase) {
    ...
    return `<div class="card"><h3>Platzierungen ${phase === 1 ? 'der Gruppen' : 'der Runden'} bestätigen</h3>
      <p class="muted small">Vorschlag nach Tabellenstand (Siege → Satzdifferenz → Punktdifferenz → direkter Vergleich). Mit ▲▼ kannst du die Reihenfolge ändern, z. B. nach Münzwurf bei Gleichstand.${anyTie ? ' <strong>Achtung: Es gibt Gleichstände (⚖).</strong>' : ''}</p>
  ...
  function renderGames() {
    ...
    <p class="muted small">Ergebnisse hier direkt eintragen oder korrigieren (Sätze eingeben, dann „Speichern“). Schiri-Zuteilungen können je Spiel überschrieben werden; „(n)“ = bisherige Einsätze.</p>
    ...
    ${games.map((g) => `<tr class="${g.status}"><td>${esc(g.time)}</td><td>${esc(g.fieldName)}</td><td>${esc(g.title)}</td>
      ...
      <td><a href="/g/${g.token}" target="_blank" title="Schiri-Seite öffnen">Link</a></td></tr>`).join('')}
    </tbody></table></div>`;
  }
  ...
  const actions = {
    ...
    confirm1: async () => { view = await api('POST', '/api/admin/phase1/confirm', { placements: placementDraft && placementDraft.phase === 1 ? placementDraft.order : undefined }); placementDraft = null; toast('2. Phase gestartet.'); },
    confirm2: async () => { view = await api('POST', '/api/admin/phase2/confirm', { placements: placementDraft && placementDraft.phase === 2 ? placementDraft.order : undefined }); placementDraft = null; toast('Finale freigegeben.'); },
    reopen1: async () => { if (!confirm('1. Gruppenphase wirklich wieder öffnen?')) return; view = await api('POST', '/api/admin/phase1/reopen', {}); toast('1. Gruppenphase geöffnet.'); },
    reopen2: async () => { if (!confirm('2. Phase wirklich wieder öffnen?')) return; view = await api('POST', '/api/admin/phase2/reopen', {}); toast('2. Phase geöffnet.'); },
    resetRefs: async () => { if (!confirm('Automatische Schiri-Zuteilungen neu berechnen? Manuelle (✎) bleiben erhalten.')) return; view = await api('POST', '/api/admin/referees/reset', {}); toast('Schiris neu verteilt.'); },
    export: async () => {
      const res = await fetch('/api/admin/export', { headers: { 'X-Admin-Pin': pin } });
      const blob = await res.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `turnier-${new Date().toISOString().slice(0, 10)}.json`; a.click();
    },
    reset: async () => {
      const confirmText = document.getElementById('resetConfirm').value;
      await api('POST', '/api/admin/reset', { confirm: confirmText });
      view = null; placementDraft = null; await refresh(); toast('Turnier gelöscht.');
    },
  };
  ...
  function bind() {
    ...
    app.querySelectorAll('[data-act]').forEach((b) => b.addEventListener('click', async () => {
      try { await actions[b.dataset.act](); render(); } catch (err) { toast(err.message, true); }
    }));
    ...
    app.querySelectorAll('[data-save]').forEach((b) => b.addEventListener('click', async () => {
      ...
      try { view = await api('POST', '/api/admin/result', { gameId: id, sets }); toast('Ergebnis gespeichert.'); render(); } catch (err) { toast(err.message, true); }
    }));
    app.querySelectorAll('[data-clear]').forEach((b) => b.addEventListener('click', async () => {
      if (!confirm('Ergebnis wirklich löschen?')) return;
      try { view = await api('DELETE', `/api/admin/result/${b.dataset.clear}`); toast('Ergebnis gelöscht.'); render(); } catch (err) { toast(err.message, true); }
    }));
    app.querySelectorAll('[data-ref]').forEach((s) => s.addEventListener('change', async () => {
      try { view = await api('POST', '/api/admin/referee', { gameId: s.dataset.ref, refereeId: s.value || null }); toast('Schiri gesetzt.'); render(); } catch (err) { toast(err.message, true); }
    }));
    app.querySelectorAll('[data-saveteam]').forEach((b) => b.addEventListener('click', async () => {
      const id = b.dataset.saveteam;
      const val = (k) => app.querySelector(`[data-team="${id}|${k}"]`).value;
      try { view = await api('POST', '/api/admin/team', { teamId: id, name: val('name'), club: val('club'), contact: val('contact') }); toast('Team gespeichert.'); render(); } catch (err) { toast(err.message, true); }
    }));
    ...
    if (settings) settings.addEventListener('submit', async (e) => {
      ...
      const body = { name: fd.get('name'), date: fd.get('date'), startTime: fd.get('startTime'), slotMinutes: Number(fd.get('slotMinutes')), fieldNames: view.tournament.fields.map((f, i) => fd.get(`field${i}`)) };
      try { view = await api('POST', '/api/admin/settings', body); toast('Gespeichert.'); render(); } catch (err) { toast(err.message, true); }
    });
  ...
  (async () => {
    if (pin) { try { await api('POST', '/api/admin/login', {}); } catch (e) { pin = ''; } }
    await refresh();
  })();
